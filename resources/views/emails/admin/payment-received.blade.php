...
<body>
    <div class="wrapper">
        ...
        <div class="body">
            <h2>Pagamento registado com sucesso</h2>
            <p>Um pagamento foi confirmado e a inscrição foi activada.</p>
            <div class="info-box">
                <p><strong>Aluno:</strong> {{ $payment->enrollment->user->name }}</p>
                <p><strong>Curso:</strong> {{ $payment->enrollment->course->name }}</p>
                <p><strong>Valor:</strong> Kz {{ number_format($payment->amount, 2, ',', '.') }}</p>
                <p><strong>Método:</strong> {{ $payment->method->getLabel() }}</p>
                @if ($payment->reference)
                    <p><strong>Referência:</strong> {{ $payment->reference }}</p>
                @endif
                <p><strong>Data de pagamento:</strong> {{ $payment->paid_at?->format('d/m/Y \à\s H:i') }}</p>
            </div>
            <a href="{{ config('app.url') }}/admin/payments" class="btn">Ver Pagamento</a>
        </div>
        <div class="footer">
